...
[Run]
; Trust the SSL certificate (prompts user for permission)
...
begin
  ...
  begin
    // Remove certificate from store using certutil
    CertPath := ExpandConstant('{app}\certs\localhost.pem');
    Exec('certutil.exe', '-delstore -user Root "' + CertPath + '"', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
    // Remove startup task
    RemoveStartupTask();
  end;
end;
